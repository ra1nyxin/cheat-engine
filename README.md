...
每次推送到 `main` 或 `master` 时，GitHub Actions 会构建 32 位、通用 64 位和 AVX2 64 位程序，并更新 `ci-latest` 预发布中的压缩包与 SHA-256 校验文件。
...
- `DBKKernel/DBKKernel.sln`：内核模式功能。驱动需要自行处理签名；未签名驱动需要在允许加载未签名驱动的环境中使用。
